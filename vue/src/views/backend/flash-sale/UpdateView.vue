        ...
        <form @submit.prevent="submitForm">
          <div class="grid grid-cols-12 gap-6">
            ...
            <div class="col-span-9 space-y-6">
              ...
              <!-- Display Selected Products with Variants -->
              <div v-if="selectedProducts.length" class="mt-4 flex flex-col space-y-2">
                <div
                  v-for="(product, index) in selectedProducts"
                  :key="product.id"
                  class="flex flex-col space-y-2 rounded-lg bg-blue-100 p-2"
                >
                  <div class="flex items-center">
                    <img :src="product.image" alt="" class="mr-2 h-8 w-8" />
                    <span class="flex-1 font-semibold text-blue-700">{{ product.name }}</span>
                    <button @click="removeProduct(index)" class="text-red-500 hover:text-red-700">
                      &times;
                    </button>
                  </div>

                  <!-- Hiển thị các biến thể của sản phẩm -->
                  <div
                    v-for="(variant, variantIndex) in flashSaleData.productVariants"
                    :key="variant.id"
                    class="ml-4 mt-2 flex items-center space-x-2 rounded bg-white p-2 shadow-sm"
                  >
                    <img :src="variant.image" alt="" class="mr-2 h-6 w-6" />
                    ...
                    <input
                      ...
                      @blur="
                        handleBlur(`products.${index}.productVariants.${variantIndex}.sale_price`)
                      "
                      @input="validateProductField('sale_price', index)"
                      class="w-24 rounded-md border px-2 py-1 text-sm"
                    />
                    <span
                      v-if="
                        form.errors.products &&
                        form.errors.products[index]?.productVariants?.[variantIndex]?.sale_price
                      "
                    ...
                    <input
                      ...
                      v-model="variant.max_quantity"
                      @blur="
                        handleBlur(`products.${index}.productVariants.${variantIndex}.max_quantity`)
                      "
                      @input="validateProductField('max_quantity', index)"
                      class="w-24 rounded-md border px-2 py-1 text-sm"
                    />
                    <span
                      v-if="
                        form.errors.products &&
                        form.errors.products[index]?.productVariants?.[variantIndex]?.max_quantity
                      "
                      class="text-xs text-red-500"
                    >
                      {{ form.errors.products[index].productVariants[variantIndex].max_quantity }}
...
const { getAll, create, getOne, messages } = useCRUD();
...
// Fetch Flash Sale details by ID
const fetchFlashSaleDetails = async (id) => {
  try {
    const response = await getOne('flash-sales', id);
    const flashSaleData = response;

    console.log(flashSaleData);
    
  } catch (error) {
    console.error('Không thể lấy chi tiết Flash Sale:', error);
  }
};
...
const submitForm = async (exitAfterSave = false) => {
  ...
  try {
    ...
    if (exitAfterSave) {
      if (!response) return (state.error = formatMessages(messages.value));
...
onMounted(() => {
  ...
  // Fetch existing flash sale data by ID from the URL
  const flashSaleId = route.params.id;
  fetchFlashSaleDetails(flashSaleId);
});
</script>
