        ...
        <div class="mx-10 mb-5 min-h-screen">
          <div class="flex items-center justify-between py-5">
            <div class="flex items-center">
              ...
              <h2 class="text-[18px] uppercase leading-none">TOP KHÁCH HÀNG THÂN THIẾT</h2>
            </div>
            <div>
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.dataIndex === 'average_spent'">
              {{ formatCurrency(record.average_spent) }}
            </template>
          </template>
          </a-table>
        </div>
...
import { formatCurrency } from '@/utils/format';
  ...
  const fetchData = async () => {
    ...
    try {
      const { data } = await axios.get('/statistics/loyal-customers', {
        params: {
          date: date.value,
